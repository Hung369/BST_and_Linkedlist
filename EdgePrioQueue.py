...
class IndexMinPQ:
    ...
    def __init__(self, mx):
        if mx < 0:
            raise ValueError(
                'Minimum size of an IndexMinPQ must be greater than 0')
        self.mx = mx
        self.n = 0
        self.pq = [None] * (mx+1)
        self.keys = [None] * (mx+1)
    ...
    def __keyContainmentCheck(self, i):
        if self.contains(i):
            raise ValueError(
                'provided index is already contained within the priority queue')

    def __missingKeyCheck(self, i):
        if not self.contains(i):
            raise ValueError(
                'Provided key is not contained within the priority queue')
    ...
    def decreaseKey(self, i, key):
        ...
        if (self.keys[i] < key):
            raise ValueError(
                'Calling decreaseKey with given argument will not strictly decrease the key')
        self.keys[i] = key
        self._swim(self.qp[i])
    ...
    def increaseKey(self, i, key):
        ...
        if self.keys[i] > key:
            raise ValueError(
                'Calling increaseKey with given argument will not strictly increase the key')
